...
import com.heroku.nwl.service.WorkSchedule;
...
public class KeyboardServiceImpl implements KeyboardService {
    ...
    public InlineKeyboardMarkup getScheduleDays() {
        ...
        for (int i = 0; i < workDays.size(); i++) {
            ...
            String month = date.getMonth().getValue() < 10 ? DOT_ZERO + date.getMonth().getValue() : DOT + date.getMonth().getValue();
            String buttonText = WEEK_DAYS[date.getDayOfWeek().getValue() - 1] + EMPTY_DATA + date.getDayOfMonth() + month;
            Map<String, String> navigateDate = getNextDate(workDays, i);
            String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_NAVIGATE_DATA, SELECT_SERVICE, date.format(FORMATTER), navigateDate.get(PRIVIES), navigateDate.get(NEXT)));
    ...
    public InlineKeyboardMarkup changeReservationStatus(Reservation reservation) {
        ...
        String dataCame = String.format(JSON_CHANGE_RESERVATION_STATUS, CHANGE_RESERVATION_STATUS, reservation.getOrderId(), ReservationStatus.SUCCESSES);
        String dataMissed = String.format(JSON_CHANGE_RESERVATION_STATUS, CHANGE_RESERVATION_STATUS, reservation.getOrderId(), ReservationStatus.MISSED);
    ...
    public InlineKeyboardMarkup getAvailableTimeKeyboard(ButtonDto dto) throws CustomBotException {
        ...
        if (dto.getPreviousDate() != null) {
            ...
            String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_SELECT_SERVICE, WORKDAY, dto.getPreviousDate().format(FORMATTER), navigateDate.get(PRIVIES), navigateDate.get(NEXT), dto.getServiceId()));
        ...
        if (dto.getNextDate() != null) {
            ...
            Map<String, String> navigateDate = getNextDate(workDays, index);
            String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_SELECT_SERVICE, WORKDAY, dto.getNextDate().format(FORMATTER), navigateDate.get(PRIVIES), navigateDate.get(NEXT), dto.getServiceId()));
    ...
    private void resizeMarkup(List<LocalTime> workTime, List<List<InlineKeyboardButton>> rowList, ButtonDto dto) {
        if (workTime.size() > 15) {
            ...
            for (LocalTime time : workTime) {
                ...
                String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_DATA_TIME, ORDER_TIME, dto.getCurrentDate().format(FORMATTER), time, dto.getServiceId()));
    ...
    public InlineKeyboardMarkup getCalendar(List<List<CalendarDayDto>> calendar, LocalDate date) {
        ...
        String priviesMonth = String.valueOf(new Formatter().format(JSON_COMMAND_CHANGE_MONTH, CHANGE_MONTH, date.minusMonths(1).format(FORMATTER), command));
        String nextMonth = String.valueOf(new Formatter().format(JSON_COMMAND_CHANGE_MONTH, CHANGE_MONTH, date.plusMonths(1).format(FORMATTER), command));
    ...
    public InlineKeyboardMarkup getReservationOnDate(LocalDate date) {
        ...
        for (Reservation order : reservationList) {
            List<InlineKeyboardButton> buttons = new ArrayList<>();
            String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_CANCEL_RESERVATION, CANCEL_RESERVE, order.getOrderId(), date.format(FORMATTER)));
            String text = order.getOrderTime() + EMPTY_DATA + order.getOrderDate() + EMPTY_DATA + order.getUser().getFirstName() + EMPTY_DATA + order.getUser().getPhoneNumber();
    ...
    public InlineKeyboardMarkup getUserReservation(Long chatId) {
        ...
        for (Reservation order : reservationList) {
            String outputData = String.valueOf(new Formatter().format(JSON_COMMAND_RESERVATION, DELETE_RESERVE, order.getOrderId(), order.getOrderDate().format(FORMATTER), order.getOrderTime()));
            List<InlineKeyboardButton> buttons = new ArrayList<>();
    ...
    public InlineKeyboardMarkup getServiceCatalog(ButtonDto buttonDto) throws CustomBotException {
        ...
        for (ServiceCatalog service : serviceCatalogs) {
            ...
            String data = String.format(JSON_COMMAND_SELECT_SERVICE, WORKDAY, buttonDto.getCurrentDate().format(FORMATTER), previousDate, nextDate, service.getServiceId());
